...
1. 能够使用jQuery 中的 on 方法绑定事件

2. 能够使用on完成事件委派

3. 能够知晓on可以给动态创建的元素绑定事件

4. 能够自己去网上下载并使用jQuery插件

5. 能够在页面中使用本地存储中的数据

6. 能够说出本地存储以及使用时数据格式如何转换

7. 能够跟随课堂完成todolist案例

## 1.1. jQuery 事件注册

​	jQuery 为我们提供了方便的事件注册机制，使开发人员易于操作优缺点如下：
...
https://github.com/alvarotrigo/fullPage.js/tree/master/lang/chinese#fullpagejs 中文
...
const renderList = function () {
    ...
    // let doneCount = 0
    console.log(res)
    // const tempLi = `<li><input type="checkbox"><p>${res.msg}</p><a href="javascript:;" id="0"></a></li>`
    $.each(res, function (i, el) {
        if (el.done) {
            doneCount++
            $('ul').prepend(`<li><input type="checkbox" checked><p>${el.msg}</p><a href="javascript:;" id="${i}"></a></li>`)
        } else {
            todoCount++
            $('ol').prepend(`<li><input type="checkbox"><p>${el.msg}</p><a href="javascript:;" id="${i}"></a></li>`)
        }
    })
...
$('ol, ul').on('click', 'a', function () {
    ...
    console.log(index)
  // 数组的splice() mdn, 返回被删除的元素  splice demo
    res.splice(index, 1) // 删除 mdn
...
// 点击复选框, 判断已完成还是进行中 done false true
$('ul, ol').on('click', 'input', function(){
    var res = getLocal()
    var index = $(this).siblings('a').attr('id')
    // console.log(index)
    res[index].done = $(this).prop('checked')
    // console.log(res)
    setLocal(res)
